...
class ImageEventHandler(FileSystemEventHandler):
    ...
    def __init__(self, callback):
        ...
        """
        ProgressLogger.log_step("ImageEventHandler 초기화됨")
    ...
    def on_created(self, event):
        ...
        """
        ProgressLogger.log_step(f"이벤트 발생: {event.src_path}")  # 모든 생성 이벤트 로깅
        # 디렉터리인 경우 무시
        ...
        if file_path.suffix.lower() in {'.jpg', '.jpeg', '.png'}:
            ProgressLogger.log_step(f"새 이미지 감지: {file_path}")
...
class DirectoryWatcher:
    ...
    def __init__(self, directory_path):
        ...
        self.observer = None
        ProgressLogger.log_step(f"감시 대상 디렉터리: {self.directory_path}")
    ...
    def start(self, callback_function):
        ...
        # 감시 시작
        self.observer.start()
        ProgressLogger.log_step("Observer 시작됨")
    ...
    def stop(self):
        ...
        if self.observer:
            ...
            self.observer.join()
            ProgressLogger.log_step("디렉터리 감시 중지")